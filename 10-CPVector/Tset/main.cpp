...
bool test1() {
  ...
  int i = n-1;
  for (CP::vector<int>::iterator it = myvector.begin(); it != myvector.end(); it++,i--) {
    assert(*it == i);
  }
...
bool test3() {
  MyVector v;
  v.insertAA(v.begin(),TempClass("1"));
  v.insertAA(v.begin(),TempClass("2"));
  v.insertAA(v.begin(),TempClass("3"));
  v.insertAA(v.begin()+1,TempClass("xxx"));
  assert(v[0].name == "3");
  assert(v[1].name == "xxx");
  assert(v[2].name == "2");
  assert(v[3].name == "1");
