...
bool compare(const McLocation &a,const McLocation &b) {
...
int main() {
  ...
  //calculate distance
  for (vector<McLocation>::iterator it = location.begin();it != location.end(); it++) {
    it->dist = distance(lat,lon,it->lat,it->lon);
  }
